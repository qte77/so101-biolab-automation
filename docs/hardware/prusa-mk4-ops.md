...
```
CadQuery (.py) → STL → PrusaSlicer → .gcode → PrusaLink PUT → print
```

Parts live in `hardware/stl/dpette/` and `hardware/stl/so101/`.

![dPette+ mount side view](../../assets/images/dpette_mount_hook.png)
...
| `dpette_cam_arm.stl` | PLA+ | Straight cam arm, shared |
| `dpette_multi_handle.stl` | PLA+ | U-bracket mount, 8-channel |

Slicer profile: `hardware/slicer/profiles/pla_plus_02mm.ini`
