...
- **Mixed-firmware patch tool** (`src/so101/cli/patch_lerobot.py`): monkey-patches lerobot 0.4.4 to tolerate STS3215 servos with mixed firmware (3.9 + 3.10). Entry point: `so101-patch-lerobot`
- **Servo scan tool** (`src/so101/cli/scan_servos.py`): pre-calibration sanity check reporting servo IDs and firmware versions. Entry point: `so101-scan-servos`
...
- **Prusa MK4 slicer profiles** (4 new `.ini` files under `src/hardware/slicer/profiles/`): `prusa_mk4_pla_02mm` (production PLA/PLA+), `prusa_mk4_pla_prototype` (fast 0.3mm fit-check), `prusa_mk4_pla_prototype_supports` (with auto-supports), `prusa_mk4_tpu_02mm` (TPU 95A)
...
- `scan_source` optional field in `src/hardware/parts.json` for scan-derived parts — queryable provenance pointing at `hardware/scans/` inputs
...
- E2E workflow orchestration (`src/so101/workflow.py`): UC1 pipetting (single/row/col/full plate), UC2 fridge ops, UC3 tool interchange, UC4 demo mode, UC5 gantry-based pipetting
...
- `src/hardware/parts.json` manifest as single source of truth for all printable parts
- `src/hardware/render.py` unified runner — dispatches build123d or OpenSCAD per manifest
- Slicer validation script (`src/hardware/slicer/validate.py`) with graceful fallback when slicer unavailable
...
- **Scripts moved** from `scripts/` to `src/so101/cli/` as proper package entry points via hatchling
- **Integration tests refactored** to call `main()` directly (not subprocess) — coverage: 75% → 80%
- **ELN client** (`src/so101/eln_client.py`): 10 inline pyright ignores replaced with single `Any` cast on untyped `elabapi_python` import
- **Coverage gate** enforced at 80% in CI (`--cov-fail-under=80`)
- **Code / assets split**: generated STL + SVG outputs moved from `src/hardware/{stl,svg}/` to top-level `hardware/{stl,svg}/`. Code (CAD scripts, `render.py`, `slicer/`, `parts.json`) stays under `src/hardware/`. New top-level `hardware/scans/` for reference 3D scan data. `render.py`, `slicer/validate.py`, `cad/util/export.py`, `tests/_paths.py` all updated to resolve asset paths via a new `ASSETS_DIR` anchor.
- `src/hardware/README.md` restructured: removed duplicated Parts Table (→ points at `parts.json` as single source of truth + `jq` query examples) and duplicated layout description (→ points at `docs/architecture.md`). Added dPette+ 8-ch Mount + dPette single-channel Mount + Scan Reference Data assembly sections, Prusa MK4 slicer profile catalog.
...
- `app/` directory restructure: `src/biolab/` → `src/so101/` (library modules) + `src/so101/cli/` (CLI scripts), `src/dashboard/` → `src/dashboard/`
...
- `src/hardware/cad/util/export.py` (`export_part`): handles `ShapeList` results from build123d boolean operations via a `_to_compound` coercion helper (disconnected shapes no longer fail with `AttributeError: ShapeList has no wrapped`)
...
- `tests/hardware/test_scad_svg.py`: resolves SVG paths via `ASSETS_DIR` (top-level `hardware/svg/`) instead of `HARDWARE_DIR` (`src/hardware/svg/`) — follows the code/assets split
